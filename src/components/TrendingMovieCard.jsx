...
const TrendingMovieCard = ({ movie, poster }) => {
  const { rating, genre } = getMovieMeta(movie.name);

  return (
    <article className="group flex-shrink-0 w-[220px] sm:w-[240px] md:w-[260px] animate-fadeInUp opacity-0" style={{ animationFillMode: 'forwards' }}>
      <div className="relative rounded-2xl overflow-hidden bg-bg-card border border-border transition-all duration-500 ease-out hover:border-border-hover hover:shadow-xl hover:-translate-y-2">
        {/* Poster */}
        <div className="relative aspect-poster overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 space-y-3">
            ...
            {/* Book button — visible on hover, always on touch */}
            <Link
              to={`/movie/${movie._id}`}
              className="inline-flex items-center justify-center gap-2 w-full btn-primary py-2.5 text-xs
